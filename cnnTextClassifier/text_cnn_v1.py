    ...
    def __init__(
      ...
      embedding_size, filter_sizes, num_filters_layer1, num_filters_layer2, l2_reg_lambda=0.0, weights_array=1.0):
        ...
        with tf.name_scope("weighted_accuracy"):
            ...
            weighted_correct_predictions_temp = tf.matmul(self.input_y, class_weight)
            ...
            weighted_correct_predictions = tf.multiply(weighted_correct_predictions_temp, transformed_correct_predictions)
            # self.weighted_accuracy = tf.reduce_mean(weighted_correct_predictions, name="weighted_accuracy")
            self.weighted_accuracy = tf.divide(tf.reduce_sum(weighted_correct_predictions), tf.reduce_sum(weighted_correct_predictions_temp), name="weighted_accuracy")