...
def train(config, model=TextCNN):
    ...
    datasets_val = data_helpers.get_datasets(data_path=config.development_path, vocab_tags_path=config.tags_vocab_path,
                                             sentences=config.data_column, tags=config.tags_column)

    # Transforming labels to one-hot vectors and apply padding or truncate according to max_document_length
    x_text, y_train = data_helpers.load_data_labels(datasets)
    x_temp, y_dev = data_helpers.load_data_labels(datasets_val)
    ...
    x_dev = np.array(list(vocab_processor.fit_transform(x_temp)))
    ...
    if config.enable_char:
        ...
        char_ids_train = numpy.delete(char_ids, numpy.s_[config.doc_length:], 1)
        word_lengths_train = numpy.delete(word_lengths, numpy.s_[config.doc_length:], 1)

        for sentence in x_temp:
            words_raw = sentence.strip().split(" ")
            words = [processing_word(w) for w in words_raw]
            char_ids += [words]
        char_ids_temp, word_lengths_temp = data_helpers.pad_sequences(config.doc_length, char_ids, pad_tok=0, nlevels=2)
        char_ids_dev = numpy.delete(char_ids_temp, numpy.s_[config.doc_length:], 1)
        word_lengths_dev = numpy.delete(word_lengths_temp, numpy.s_[config.doc_length:], 1)
