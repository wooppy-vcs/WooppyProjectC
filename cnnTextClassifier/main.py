...
if __name__ == "__main__":
    ...
    #         for x in len:
    #             if a == LSTMCNN:
    #                 config = Config(run_number=y, enable_char=True, transfer_learning=True, doc_length=x,
    ...
    #             eval_op.evaluation(config=config)
    config = Config(run_number=1, enable_char=True, transfer_learning=True, doc_length=50,
                    model_name='LSTM-CNN', checkpoint_dir="")
    train_op.train(config=config, model=LSTMCNN)
